...
import { getMarketingArticle } from "@/lib/marketing-seo";
import { getPublishedMarketingPostBySlug } from "@/src/lib/marketing-supabase-server";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const article = getMarketingArticle(slug);
  ...
  const post = await getPublishedMarketingPostBySlug(slug);

  if (!post) {
    return createMetadata("Guiden kunde inte hittas", "Den efterfrågade guiden finns inte.");
  }

  return createMetadata(post.seo_title ?? post.title, post.seo_description ?? post.excerpt ?? "Praktisk vägledning från Altura Nova.", {
...
export default async function ArticlePage({ params }: Props) {
  const slug = (await params).slug;
  const article = getMarketingArticle(slug);
  const post = article ? null : await getPublishedMarketingPostBySlug(slug);

  if (!article && !post) notFound();

  const title = article?.title ?? post!.title;
  const description = article?.description ?? post!.seo_description ?? post!.excerpt ?? "Praktisk vägledning från Altura Nova.";
        ...
        <SectionContainer>
          <header className="mx-auto max-w-4xl">
            <div className="flex flex-wrap items-center gap-3 text-xs font-bold tracking-[0.16em] text-[#e86f44] uppercase">
              ...
              <span className="text-[#75827c]">{article?.readTime ?? "Guide"}</span>
            </div>
          ...
          <div className="mx-auto mt-12 grid max-w-6xl gap-10 lg:grid-cols-[minmax(0,1fr)_17rem] lg:items-start">
            <div className="rounded-[2.25rem] border border-[#173f35]/10 bg-[#fffdf8] p-7 md:p-10">
              {article ? (
                ...
                <BlogRichText content={post!.content} className="text-[#45564f] [&_h2]:text-[#173f35] [&_h3]:text-[#173f35] [&_a]:text-[#173f35]" />
              )}
            </div>
