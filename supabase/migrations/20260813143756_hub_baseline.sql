...
create or replace function public.create_hub_organization(
  target_name text,
  target_org_number text default null,
...
begin
  ...
  if char_length(trim(coalesce(target_name, ''))) not between 1 and 160 then
    raise exception 'Organization name is invalid';
  end if;

  insert into public.organizations (id, name, org_number, email)
  values (
    target_organization_id,
    trim(target_name),
    nullif(trim(target_org_number), ''),
    nullif(trim(target_email), '')
  );

  insert into public.organization_members (organization_id, user_id, role)
  values (target_organization_id, actor_id, 'owner');

  insert into public.activity_log (
    organization_id, user_id, action, description
  ) values (
    target_organization_id,
    actor_id,
    'organization_bootstrapped',
    'Första hubborganisationen skapades automatiskt.'
  );
...
declare
  ...
  actor_role text;
begin
  actor_role := public.user_org_role(target_organization_id);

  if actor_role is null or actor_role not in ('owner', 'admin', 'member') then
...
revoke execute on function public.create_hub_organization(text, text, text) from public;
...
grant execute on function public.create_hub_organization(text, text, text) to authenticated;
grant execute on function public.claim_next_invoice_number(uuid) to authenticated;
